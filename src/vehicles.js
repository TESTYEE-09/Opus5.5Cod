...
export const SPEC = {
  ...
  gunship: { name: 'Attack Helicopter', hp: 1600, armor: 0.35, bounty: 300, seat: 'inside', air: true, size: 2.6, boom: 1.8, splash: { Flak: 0.5 } },
...
const HELP = {
  ...
  gunship: 'Mouse aim + turn · W S forward / back · A D strafe · Space climb · Shift descend · LMB 30 mm · R ATGM at the crosshair · G rocket pod · RMB zoom · F exit',
...
function hitKind(kind, v, o, d, maxT) {
  ...
  else if (kind === 'heli' || kind === 'gunship') return chopperHit(v.pos, o, d, maxT);
...
function pose(kind, m, pos, quat, a, b, dt) {
  ...
  else if (kind === 'heli' || kind === 'gunship') { m.rotor.rotation.y += dt * 28; m.tail.rotation.x += dt * 35; }
...
export class FighterJet extends Vehicle {
  constructor(game, owner, id, kind = 'jet') {
    ...
    const dist = owner.isPlayer ? 2400 : 6000;
    this.pos.set(SIZE / 2 + Math.sin(a) * dist * (team0 ? 1 : -1), owner.isPlayer ? 450 : 700, SIZE / 2 + Math.cos(a) * dist * (team0 ? -1 : 1));
    ...
    this.aimYaw = yaw; this.aimPitch = 0; this.rollIn = 0; this.zoom = 0; this.zoomWant = false;
    this.agmLock = null; this.agmLockT = 0;
  ...
  // Arcade mouse aim: the mouse moves an aim point freely and an instructor flies the jet at
  // it (bank to turn, pull, a touch of rudder). No stall, no fuel, and it will not fly you into
  // the ground.
  control(dt, inp) {
    const s = this.game.settings.sens * 0.0022 * (this.zoom > 0.5 ? 0.45 : 1);
    this.aimYaw -= inp.dx * s;
    this.aimPitch = clamp(this.aimPitch - inp.dy * s, -1.35, 1.35);
    this.rollIn = inp.right - inp.left;
    this.throttle = clamp(this.throttle + (inp.forward - inp.back) * 0.6 * dt, 0.25, 1);
    this.ab = inp.sprint && this.af.ab > 0;
    this.gunWant = inp.fire; this.zoomWant = inp.ads;
  ...
  fly(dt) {
    ...
    let p;
    if (this.rollIn) p = -this.rollIn * af.roll * 0.8;
    else {
      const bank = -clamp(yawErr * 2.2, -1.35, 1.35) * clamp(off * 5, 0, 1) - (off < 0.05 ? roll * 0.8 : 0);
      p = clamp((bank - roll) * 4, -af.roll, af.roll);
    }
    const q = clamp(pitchErr * 3.2, -1.1, 1.4) * perf;
    rotLocal(this.quat, AX, q * dt);
    rotLocal(this.quat, AZ, p * dt);
    rotLocal(this.quat, AY, -clamp(yawErr * 1.4, -0.5, 0.5) * dt);
    this.quat.normalize();
    const f = fwdOf(this.quat, _f);
    const target = af.stall * 1.6 + (af.vmax - af.stall * 1.6) * this.throttle + (this.ab ? af.vab - af.vmax : 0);
    this.speed = clamp(this.speed + ((target - this.speed) * 0.6 - 9.81 * f.y * 0.5) * dt, af.stall * 1.3, af.vab * 1.05);
    this.gLoad = q * this.speed / 9.81 + 1; this.gStrain = 0; this.stalled = false;
  ...
  update(dt) {
    ...
    if (this.driver) this.outside = dc > 5000;
    else if (this.ai) this.think(dt);
    else this.leave(dt);
    if (!this.alive) return;
    if (this.controlled) this.fly(dt);
    else {
    ...
    this.trackAam(dt);
    if (this.controlled) this.trackAgm(dt);
  ...
  // arcade seeker: whatever ground target sits nearest the aim point, within 10 degrees and 4 km
  trackAgm(dt) {
    if (this.agm <= 0) { this.agmLock = null; return; }
    const d = aimDir(this.aimYaw, this.aimPitch, _z);
    let best = null, bd = Math.cos(10 * DEG);
    for (const e of this.game.targetsFor(this.team)) {
      if (e.air) continue;
      const v = e.aimPoint(_v, false).sub(this.pos), r = v.length();
      if (r > 4000 || r < 250) continue;
      const cth = d.dot(v.divideScalar(r)) + (e.isVehicle ? 0.004 : 0);
      if (cth > bd) { bd = cth; best = e; }
    }
    if (best && best === this.agmLock) this.agmLockT += dt; else { this.agmLock = best; this.agmLockT = 0; }
  }

  fireAgm() {
    if (this.controlled && this.agm > 0) {
      const d = aimDir(this.aimYaw, this.aimPitch, new THREE.Vector3());
      if (this.agmLock && this.agmLockT > 0.5) { this.podTarget = this.agmLock; this.podPoint = this.agmLock.aimPoint(new THREE.Vector3(), false); }
      else {
        const h = raycastWorld(this.pos, d, 6000);
        if (!h && d.y > -0.02) { this.game.hud.toast('Aim at the ground or a target first'); return; }
        this.podTarget = null;
        this.podPoint = this.pos.clone().addScaledVector(d, h ? h.t : this.pos.y / -d.y);
      }
    }
    ...
    if (range > 9000 || f.dot(to.normalize()) < (this.controlled ? -0.2 : 0.5)) { if (this.controlled) this.game.hud.toast('Target out of the missile\'s field of view'); return; }
    this.agm--;
  ...
  trackAam(dt) {
    ...
    const f = this.controlled ? aimDir(this.aimYaw, this.aimPitch, _f) : fwdOf(this.quat, _f);
    let best = null, bd = -1;
    for (const v of this.game.vehicles) {
  ...
  fireAam() {
    ...
    const t = this.aamLock && this.aamLockT > (this.controlled ? 0.5 : 0.8) ? this.aamLock : null;
    if (!t) { if (this.controlled) this.game.hud.toast('No lock: keep the target in the seeker circle'); return; }
    this.aam--;
    const side = this.aam % 2 ? 1 : -1;
    const f = fwdOf(this.quat, _f), o = this.pos.clone().add(_u.set(4 * side, -0.5, 0).applyQuaternion(this.quat));
  ...
  view(cam, dt) {
    ...
    if (this.controlled) {
      // chase camera on the aim line, level with the horizon
      this.zoom += ((this.zoomWant ? 1 : 0) - this.zoom) * Math.min(1, dt * 8);
      const d = aimDir(this.aimYaw, this.aimPitch, _f);
      const want = _v.copy(d).multiplyScalar(-26).add(_u.set(0, 6.5, 0));
      if (!this.camInit) { this.camOff.copy(want); this.camInit = true; }
      this.camOff.lerp(want, Math.min(1, dt * 7));
      cam.position.copy(this.pos).add(this.camOff);
      _m.lookAt(cam.position, _c.copy(this.pos).addScaledVector(d, 160), AY);
      cam.quaternion.setFromRotationMatrix(_m);
      return this.zoom > 0.5 ? 32 : clamp(this.game.settings.fov - 5 + (this.speed - 150) * 0.04, 60, 100);
    }
  ...
  drawHud(ctx, W, H, P, game) {
    ...
    if (this.controlled) { this.arcadeHud(ctx, W, H, P, game); return; }
  ...
  arcadeHud(ctx, W, H, P, game) {
    const green = 'rgba(140,255,160,0.95)', gold = '#ffd24a', cx = W / 2, af = this.af;
    drawMarkers(ctx, game, P, { range: 3000 });
    // where the mouse aims (the jet flies there) and where the guns point now
    const aim = P(_c.copy(this.pos).addScaledVector(aimDir(this.aimYaw, this.aimPitch, _v), 1000));
    if (aim) ring(ctx, aim[0], aim[1], 11, 'rgba(255,255,255,0.85)', 2);
    const gun = P(_c.copy(this.pos).addScaledVector(fwdOf(this.quat, _f), 700));
    if (gun) cross(ctx, gun[0], gun[1], 16, 5, green, 2);
    const lockBox = (e, t, need, label) => {
      const s = P(e.aimPoint(_c, false));
      if (!s) return;
      const locked = t > need;
      box(ctx, s[0], s[1], locked ? 20 : 26 + Math.sin(game.time * 20) * 3, locked ? gold : green);
      text(ctx, locked ? label : 'LOCKING', s[0], s[1] - 32, 'center', locked ? gold : green, 15);
    };
    if (this.agmLock) lockBox(this.agmLock, this.agmLockT, 0.5, `LOCK · R ${af.agm.name}`);
    if (this.aamLock) lockBox(this.aamLock, this.aamLockT, 0.5, `LOCK · G ${af.aam.name}`);
    const agl = this.pos.y - Math.max(0, terrainY(this.pos.x, this.pos.z));
    text(ctx, `${af.name}   ${Math.round(this.speed * 3.6)} km/h   ALT ${Math.round(agl)} m   THR ${Math.round(this.throttle * 100)}%${this.ab ? ' AB' : ''}`, cx, 112, 'center', green, 20);
  ...
  stats() {
    return `<div>${this.af.name}</div><div>GUN <b>${this.rounds}</b> · MISSILES <b>${this.agm + this.aam}</b></div>`;
  }
}
...
// ---------- attack helicopter the player flies (tech tree) ----------
// Arcade handling: it holds its height, WASD moves it relative to where you look, the mouse
// turns it and aims the chin gun. Bots fly it as a gunship over the fight.
export class AttackHeli extends Vehicle {
  constructor(game, owner, id, x, z, yaw) {
    super(game, 'gunship', owner, id);
    this.addModel();
    this.pos.set(x, terrainY(x, z) + 0.2, z);
    this.heading = this.aimYaw = yaw; this.aimPitch = -0.15; this.yaw = yaw;
    this.inp = { f: 0, s: 0, u: 0 }; this.fireWant = false; this.zoomWant = false; this.zoom = 0;
  ...
  control(dt, inp) {
    const s = this.game.settings.sens * 0.0022 * (this.zoom > 0.5 ? 0.4 : 1);
    this.aimYaw -= inp.dx * s;
    this.aimPitch = clamp(this.aimPitch - inp.dy * s, -1.3, 0.5);
    this.inp.f = inp.forward - inp.back; this.inp.s = inp.right - inp.left;
    this.inp.u = (inp.jump ? 1 : 0) - (inp.sprint ? 1 : 0);
    this.fireWant = inp.fire; this.zoomWant = inp.ads;
    if (inp.reload) this.fireAtgm();
    if (inp.nade) this.rocketWant = true; else this.rocketWant = false;
    if (inp.pronePressed && this.flares > 0) { this.flares -= 2; this.flareT = 2.5; this.game.audio.flares?.(null); }
  }

  update(dt) {
    const g = this.game;
    this.t += dt;
    this.flareT = Math.max(0, (this.flareT || 0) - dt); this.missileWarn = Math.max(0, this.missileWarn - dt);
    if (!this.driver) this.think(dt);
    if (!this.alive) return;
    // turn toward the aim, move in the heading frame, hold height
    this.heading += wrap(this.aimYaw - this.heading) * Math.min(1, dt * 2.2);
    this.yaw = this.heading;
    const fx = -Math.sin(this.heading), fz = -Math.cos(this.heading), sp = 32;
    const ground = terrainY(this.pos.x, this.pos.z);
    const wantY = this.inp.u * 9, lifting = this.pos.y - ground > 1;
    _v.set((fx * this.inp.f + -fz * this.inp.s) * sp, wantY, (fz * this.inp.f + fx * this.inp.s) * sp);
    if (!lifting && this.inp.u <= 0) _v.set(0, 0, 0);
    this.vel.lerp(_v, Math.min(1, dt * 1.6));
    const step = this.vel.length() * dt;
    if (step > 0.001) {
      const dir = _w.copy(this.vel).normalize(), hit = raycastWorld(this.pos, dir, step + 2.5);
      if (hit) { if (this.vel.length() > 18 && !lifting) this.crash(); this.vel.multiplyScalar(-0.3); }
      else this.pos.addScaledVector(this.vel, dt);
    }
    this.pos.x = clamp(this.pos.x, -300, SIZE + 300); this.pos.z = clamp(this.pos.z, -300, SIZE + 300);
    this.pos.y = clamp(this.pos.y, ground + 0.2, 260);
    // nose dips into the direction of travel, rolls into strafes
    const lf = this.vel.x * fx + this.vel.z * fz, ls = -this.vel.x * fz + this.vel.z * fx;
    this.quat.setFromEuler(_e.set(-lf * 0.012, this.heading, -ls * 0.014, 'YXZ'));
    pose('gunship', this.m, this.pos, this.quat, 0, 0, dt);
    this.sound?.set(this.pos);
  ...
  crash() {
    ...
    this.destroy(null, 'Jet', false);
    if (this.game.authority) this.game.explode(p, this.owner, 8, 200, 'Jet', { streak: true });
  }
  ...
  fireRocket() {
    this.rocketT = 0.18; this.rockets--;
    const d = this.aimVec(new THREE.Vector3()), side = this.rockets % 2 ? 1.6 : -1.6;
    const o = this.pos.clone().add(_u.set(side, -0.8, 0).applyQuaternion(this.quat));
    d.x += (Math.random() - 0.5) * 0.03; d.y += (Math.random() - 0.5) * 0.03; d.z += (Math.random() - 0.5) * 0.03;
    this.game.fireProjectile('rpg', this.driver || this.owner, o, d.normalize().multiplyScalar(90), null, this);
  }

  fireAtgm() {
    if (this.atgm <= 0) { if (this.controlled) this.game.hud.toast('No ATGMs left'); return; }
    const d = this.aimVec(new THREE.Vector3()), eye = this.pos.clone().addScaledVector(d, 4);
    const h = this.game.traceShot(eye, d, 2500, this);
    const pt = eye.clone().addScaledVector(d, h.t);
    const tgt = h.entity && h.entity.isVehicle ? h.entity : { alive: true, pos: pt, vel: new THREE.Vector3(), point: true };
    this.atgm--;
    const o = this.pos.clone().add(_u.set(this.atgm % 2 ? 1.8 : -1.8, -0.8, 0).applyQuaternion(this.quat));
    this.game.fireProjectile('atgm', this.driver || this.owner, o, d.multiplyScalar(40), tgt, this);
    if (this.controlled) this.game.hud.toast(h.entity?.isVehicle ? `ATGM away: ${h.entity.name}` : 'ATGM away');
  }

  // AI: climb to 40 m, hover over the nearest fight and hose it down
  think(dt) {
    const g = this.game;
    this.inp.f = 0; this.inp.s = 0; this.fireWant = false; this.rocketWant = false;
    const alt = this.pos.y - terrainY(this.pos.x, this.pos.z);
    ...
    if ((this.scanT -= dt) <= 0) {
      ...
      for (const e of g.targetsFor(this.team)) {
        ...
        const d = e.pos.distanceTo(this.pos);
        if (d < best && lineOfSight(this.pos, e.aimPoint(_w, false))) { best = d; this.target = e; }
      }
    }
    const t = this.target;
    ...
    if (t?.alive && alt > 15) {
      ...
      const c = SIZE / 2;
      this.aimYaw = Math.atan2(-(c - this.pos.x), -(c - this.pos.z));
      this.inp.f = Math.hypot(c - this.pos.x, c - this.pos.z) > 120 ? 0.7 : 0;
    }
  }

  view(cam, dt) {
    this.zoom += ((this.zoomWant ? 1 : 0) - this.zoom) * Math.min(1, dt * 10);
    const d = this.aimVec(_f);
    const want = _v.copy(this.pos).addScaledVector(d, -16).add(_u.set(0, 4.5, 0));
    if (!this.camInit) { this.camPos.copy(want); this.camInit = true; }
    this.camPos.lerp(want, Math.min(1, dt * 8));
    cam.position.copy(this.camPos);
    _m.lookAt(cam.position, _c.copy(this.pos).addScaledVector(d, 60), AY);
    cam.quaternion.setFromRotationMatrix(_m);
    return this.zoom > 0.5 ? 30 : this.game.settings.fov;
  }

  drawHud(ctx, W, H, P, game) {
    drawMarkers(ctx, game, P, { range: 600 });
    const green = 'rgba(160,255,170,0.95)';
    const s = P(_c.copy(this.pos).addScaledVector(this.aimVec(_f), 300));
    if (s) { ring(ctx, s[0], s[1], 16, green); cross(ctx, s[0], s[1], 10, 4, green); }
    const alt = this.pos.y - terrainY(this.pos.x, this.pos.z);
    text(ctx, `${this.name.toUpperCase()}   ALT ${Math.round(alt)} m   SPD ${Math.round(this.vel.length() * 3.6)} km/h`, W / 2, 112, 'center', green, 20);
    text(ctx, `30MM · ATGM x${this.atgm} · ROCKETS x${this.rockets} · FLARES ${this.flares}`, W / 2, H - 36, 'center', green, 18);
    if (this.missileWarn > 0 && Math.floor(game.time * 4) % 2 === 0) text(ctx, 'MISSILE LAUNCH · CTRL FLARES', W / 2, H * 0.24, 'center', '#ff6a50', 26);
  }

  stats() { return `<div>ATGM <b>${this.atgm}</b> · ROCKETS <b>${this.rockets}</b></div>`; }
}
